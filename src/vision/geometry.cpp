...
Result<TransformMetadata> ComputeLetterboxTransform(int source_width, int source_height,
    ...
    return Result<TransformMetadata>::Success(
        TransformMetadata{source_width, source_height, destination_width, destination_height,
                          resized_width, resized_height, remaining_width / 2, remaining_height / 2,
...
RectF MapBoxToSource(const RectF& model_box, const TransformMetadata& transform) {
    ...
    const float left = (model_box.x - static_cast<float>(transform.pad_left)) / transform.scale_x;
    const float top = (model_box.y - static_cast<float>(transform.pad_top)) / transform.scale_y;
    const float right = (model_box.x + model_box.width - static_cast<float>(transform.pad_left)) /
                        transform.scale_x;
    const float bottom = (model_box.y + model_box.height - static_cast<float>(transform.pad_top)) /
                         transform.scale_y;
