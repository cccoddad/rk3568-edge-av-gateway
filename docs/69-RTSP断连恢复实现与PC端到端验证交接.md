...
- 重连尝试在 worker 线程内同步执行。只读核对 FFmpeg 6.1 后确认：TCP 建连默认每地址 5 秒，
  但握手读取和流式写入默认无超时；该缺口已由 70 号交接的 `rtsp_timeout_ms` 中断回调守卫
  解决（黑洞场景 3000 ms 配置 3074 ms 精确失败）。
...
- 鉴权、TLS 和跨网段丢包场景未验证；UDP 传输与超时守卫已在 70 号交接单独验证。
