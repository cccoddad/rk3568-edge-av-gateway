...
2026-08-22 执行 `sync` 和 `poweroff`，物理断电至少 10 秒后重新上电。`eth0` 自动恢复
`192.168.50.2/24`，证明 SysV 网络脚本已正确加载配置。重新插接网线后，板端
`carrier=1`、`LOWER_UP`；Windows 有线网卡为 `Up` 且配置 `192.168.50.1/24`。Windows 到板端
`ping` 4/4 成功、ARP 表得到板端 MAC `de:5f:36:04:cb:95`；板端邻居表也得到 Windows MAC，
因此静态 IP 和双向二层链路已完成冷启动验收。

板端到 Windows 的 ICMP 没有回包，但邻居状态为 `REACHABLE`；这是 Windows 入站 ICMP 防火墙
策略，不能据此否定网络链路。Windows 到板端的 ICMP 与 TCP 验证已经成功。

此次 `poweroff` 最终到达内核 `reboot: Power down`，说明关机路径完成；其中 `/dev` busy 和 Wi-Fi
驱动停止日志已保留。此前 `reboot` 停在 `usbdevice stop` 附近的问题尚未复现定位，仍作为独立风险。
...
### 3.7 M5 30 分钟与 2 小时 Mock 长稳

在没有摄像头、麦克风和网络依赖的条件下，使用 `soak_test_buildroot.sh` 继续完成 M5 长稳。
每轮均先校验 `mock.json`，并将配置校验、测试和长稳日志保存在板端 `/root/rkav/out/`。

| 项目 | 30 分钟 | 2 小时 |
|---|---:|---:|
| 证据目录 | `m5-30min-20260822-030420/soak` | `m5-2hour-20260822-060200/soak` |
| 脚本退出码 | 0 | 0 |
| 热保护触发 | 0 | 0 |
| CSV 行数 | 180（表头 + 179 次采样） | 241（表头 + 240 次采样） |
| 最后采样时间 | 1793 s | 7188 s |
| RSS | 4360 kB，稳定 | 4376 kB，稳定 |
| 线程数 | 9，稳定 | 9，稳定 |
| CPU 占用 | 约 80%～85% | 约 82%～89% |
| SoC 温度 | 约 46.7～47.8 C | 约 45.6～46.7 C |
| 音频捕获/编码 | 90001 / 90001 | 360002 / 360002 |
| 视频捕获/编码 | 54001 / 54001 | 216001 / 216001 |
| 推理请求/结果 | 54001 / 54001 | 216001 / 216001 |
| 路由/消费包 | 144002 / 144002 | 576003 / 576003 |
| 错误、恢复、过期检测 | 0 / 0 / 0 | 0 / 0 / 0 |
| 队列丢弃 | 0 | 0 |
| 最终停止原因 | `run_duration_elapsed` | `run_duration_elapsed` |

两轮最终日志中的所有队列均为 `closed=true,size=0`，高水位分别为音频 1、推理 1、视频编码 1、
Sink 2，均远小于容量。这证明当前 Mock 工作负载下的内存、线程、背压和正常 Drain 停止在 RK3568
上可持续 2 小时；不代表真实外设、NPU 或硬件编码链路的性能。

同次收尾还完成 RTC 与 SSH 留档。冷启动后日期回到 1970；`/sys/class/rtc` 为空、`/dev/rtc*` 不存在，
`hwclock -r` 无法打开 RTC。内核日志显示 `rtc-pcf8563 0-0051` probe 写操作失败（`error -5`），所以
当前是 RTC 驱动/设备树或硬件通信未就绪，不能仅归因为无后备电池。长稳前以 Windows UTC 时间临时
设置板端 wall clock；程序 PTS 和超时仍使用单调时钟，不受此问题影响。

SSH 方面，Dropbear PID 以 `/usr/sbin/dropbear -R` 运行，监听 IPv4/IPv6 22 端口，主机密钥和熵池
均存在。Windows `Test-NetConnection` 成功，但 OpenSSH 9.5 在发送本地版本后出现
`Connection timed out during banner exchange`。因此网络、ARP 和 TCP 已验收，SSH banner/协议问题
仍待排查；当前以串口为主控制通道。
...
7. 在真实 RK3568 上取得配置校验、30 项测试、信号退出、30 分钟和 2 小时长稳的吞吐、延迟、
   队列、RSS、温度和 CPU 证据。

## 5. 当前边界与遗留风险

- 30 分钟和 2 小时 Mock 长稳已经通过；仍未进行产品级长期运行，也不代表真实外设工作负载。
- 板卡 RTC 在冷启动后回到 1970，且 `pcf8563` 驱动探测失败；当前只能在每次冷启动后临时校时。
...
1. 静态 IP 已通过物理断电后的冷启动验收；保留当前 `192.168.50.2/24` 配置，不改为 DHCP。
2. 获取原理图和设备树，定位 `pcf8563@0x51` probe 失败；解决前每次冷启动后校时。
3. SSH 不是接入真实硬件的前置条件，可单独调查 banner 超时，不阻塞主线。
4. 定位 `usbdevice stop` 卡顿；在解决前，关机前先 `sync` 并保留串口日志。
5. 30 分钟和 2 小时 Mock 长稳已完成；真实外设接入后仍须按相同口径重新执行 30 分钟长稳。
...
30 项板端测试、SIGINT/SIGTERM 优雅退出、30 分钟和 2 小时 Mock 双流长稳。2 小时内 576003 个包
完整路由消费，错误和丢弃均为 0，RSS 稳定在约 4.3 MiB，SoC 温度约 45～47 摄氏度，停止后队列
全部排空。静态 IP 已经冷启动验证；RTC 驱动探测和 SSH banner 超时仍待处理。下一步使用厂商 SDK
逐个接入 V4L2、ALSA、RKNN 和 MPP。
