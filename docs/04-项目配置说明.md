...
启用 `RKAV_WITH_RKNN` 后，`backend` 可设为 `rknn`。RKNN 当前接受 CPU `RGB888`、
`BGR888`，或者在同时启用 `RKAV_WITH_JPEG` 时接受 V4L2 `MJPEG`。MJPEG 只在 5 FPS 等
推理抽帧之后解码为紧密 `RGB888`，视频编码分支仍保留原始压缩帧，避免把 30 FPS 全部
解码。生产配置见 `config/rk3568-rknn-rgb.json` 和 `config/rk3568-rknn-mjpeg.json`。

## overlay

`overlay.enabled=false` 是默认值，保持既有生产链路不增加像素复制。启用后当前仅支持
`backend=cpu`：先复制 RGB888/BGR888 源帧，再绘制裁剪检测框和 `C<类别 ID> <置信度>%` 文字。
`line_width` 范围是 1 到 8；`wait_for_result_ms` 范围为 0 到 `inference.max_result_age_ms`。

只有实际成功送入推理队列的帧才会等待结果；来源 `sequence` 和 PTS 必须同时相等才会绘制，
超时、旧结果或未来结果均保持原图。MJPEG 启用 OSD 需要 `RKAV_WITH_JPEG`，以便编码支路先解码
到 RGB。后续 RGA 后端必须保持这些绑定和超时语义。

## encoders 与 outputs

默认编码器仍是 `checksum`，输出包使用明确的 `mock_video_checksum/mock_audio_checksum` codec
标识，不能送给真实播放器。启用 `RKAV_WITH_FFMPEG` 后可使用 `video_encoder.backend=ffmpeg`、
`audio_encoder.backend=ffmpeg` 和 `outputs.type=mp4`，要求 MP4 输出使用 `block_producer` 与
正的 `push_timeout_ms`，拒绝将 Checksum packet 伪装封装为 MP4。
