...
6. **3 路客户端 1 小时并发长稳（补充验证，2026-09-13）**：每个客户端持续拉流 3600 秒、
   写入 12 段 5 分钟 MP4，总时长 3600.128/3600.106/3600.087 秒，全部退出码 0；
   `readerCount` 全程保持 3（11 次采样），网关零断连、零重连、零致命错误；
   ZLM CPU 平均 7.12%（max 7.52%）、内存平均 16.19 MiB（max 16.44 MiB），网关 RSS 平稳、
   FD 恒定 5；证据目录 `zlm-multiclient-1h-20260913-092307`。
...
### 3.1 多客户端 1 小时长稳（2026-09-13）

| 指标 | 结果 |
|---|---|
| 客户端 | 3 路并发 RTSP，各 3600 秒、12 段 5 分钟 MP4，退出码 0 0 0 |
| 总时长 | 3600.128 / 3600.106 / 3600.087 秒（无缺口） |
| readers | 全程 3（11 次采样 min=max=3） |
| 网关 | 零断连/零重连/零致命错误，RSS 平稳，FD 恒定 5 |
| ZLM | CPU 平均 7.12%（max 7.52%）；内存平均 16.19 MiB（max 16.44 MiB） |
| 证据 | `zlm-multiclient-1h-20260913-092307`；`monitor.csv` `4ebdcade...6499`、`gateway.log` `d55fdd04...60f1` |
